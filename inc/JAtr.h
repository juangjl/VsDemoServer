...
#define ATR_UNREADABLE  (42)	  /// 42 U unreadable
#define ATR_UNREADABLES (43)	  /// 43 U excessive noise or signal loss occurs in the signals
#define ATR_UNREADABLEE (44)	  /// 44 U excessive noise or signal loss ceases in the signals
#define ATR_AFON      (45)	    /// 45 { start of Atrial flutter/fibrillation
#define ATR_AFOFF     (46)	    /// 46 } end of Atrial flutter/fibrillation
...
#define LABEL_UNREADABLES	 "UNREADS"				/// 43 excessive noise or signal loss occurs in the signals 
#define LABEL_UNREADABLEE	 "UNREADE"				/// 44 excessive noise or signal loss ceases in the signals
#define LABEL_AFON	   "AFON"							  /// 45 start of atrial flutter/fibrillation 
#define LABEL_AFOFF	   "AFOFF"							/// 46 end of atrial flutter/fibrillation
...
///-------------------------------------------------------------------///
/// ICE Classification (IEC 60601-2-47)
///-------------------------------------------------------------------///
#define	ICE_N		(0)   ///< Normal
#define ICE_S		(1)   ///< Supraventricular
#define	ICE_V		(2)   ///< Ventricular
#define	ICE_F		(3)   ///< Fusion
#define	ICE_Q		(4)   ///< Paced/Unknown
#define ICE_O   (5)   ///< Other (False Positive)
#define	ICE_X		(6)   ///< Unreadable
...
#define ATR_FILE_ITEM_COUNT   (816)
#define ATR_FILE_SIG          (0x45110164)
typedef struct JAtrFileSt
{
	JWORD 	  iAtrId;                           ///<    0 : 2    --> Page id
  JWORD		  wAtrItemCnt;                      ///<    2 : 2    --> Total Count of ATR Item
  JDWORD    dwAtrTime;                        ///<    4 : 4    --> Start Time in UTC
  JAtrType  atrItems[ATR_FILE_ITEM_COUNT];    ///<    8 : 4080 --> 816 * 5  
  JDWORD    dwAtrSig;                         ///< 4088 : 4    --> KEY = 0x45110164
  JDWORD 	  dwAtrChkSum;                      ///< 4092 : 4    --> check sum from 00000 ~ 4091, total 4092 bytes;
} JAtrFileType;

typedef struct JAtrInfoSt
{
  JDWORD dwDID; ///< arrary index
  JBYTE  bAtr;  ///< 
  JDWORD dwUTC; ///< utc time
  JWORD  wMSec; ///< msec 
} JAtrInfoType;

#define ATR_INFO_QUEUE_SIZE     (1000)
typedef struct JAtrInfoDataBaseSt
{
  JINT iAtrCnt;
  JINT iAtrQueueHead;
  JINT iAtrQueueTail;  
  JAtrInfoType pAtrItem[ATR_INFO_QUEUE_SIZE];
}JAtrInfoDataBaseType;

#define ATR_LAST_ITEM_SIZE     (100)

typedef struct JAtrLastItemSt
{
  JINT iAtrCnt;
  JINT iUpdate;
  JINT iStatus[ATR_LAST_ITEM_SIZE];
  JAtrInfoType pAtrItem[ATR_LAST_ITEM_SIZE];
}JAtrInfoListType;

#pragma pack (pop)

extern const char * JAtrLabel[];
extern const char * strAtrAbbrev[];
